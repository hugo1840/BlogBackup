...
modified: '2023-07-01T08:56:35.307Z'
---
...
```
指定`skip-gtids=true`可以把binlog日志中已经执行过的语句当成一个新事务来执行才行，但是这样会把两个时间点中间的所有事务都重新执行一遍，可能会导致某些数据被重复插入，造成数据不一致。
...
## 增量恢复
增量恢复要使用到mysqlbinlog日志解析工具。

手动切换binlog日志文件，并检查最新的binlog文件名：
```sql
SQL> flush logs;
...
也可以使用mysqlbinlog工具解析binlog日志来分析，通过`-d`指定数据库：
```bash
...
## 直接使用binlog日志恢复
如果能很方便地找到原始的插入语句（比如被删除的记录都是集中在某一时间段内插入的），可以直接使用mysqlbinlog进行数据恢复。

关注到日志中的下面的多行内容：
```bash
SET @@SESSION.GTID_NEXT= '3e863e10-eede-11ed-a3fd-005056a3eca7:12722162'/*!*/;
# at 2006
#230625 10:37:22 server id 100  end_log_pos 2083 CRC32 0xf5466111       Query   thread_id=12    exec_time=0     error_code=0
SET TIMESTAMP=1687660642/*!*/;
BEGIN
/*!*/;
# at 2083
#230625 10:37:22 server id 100  end_log_pos 2154 CRC32 0xd6c1cc42       Rows_query
# insert into t1 (title,price) values ('RDR2',88)
# at 2154
#230625 10:37:22 server id 100  end_log_pos 2213 CRC32 0x7711d552       Table_map: `testdb`.`t1` mapped to number 125
# at 2213
#230625 10:37:22 server id 100  end_log_pos 2267 CRC32 0x588e689f       Write_rows: table id 125 flags: STMT_END_F
...
# at 2267
#230625 10:37:22 server id 100  end_log_pos 2298 CRC32 0xfb9bc314       Xid = 208
COMMIT/*!*/;
# at 2298
```
为被删除行对应的插入语句，事务起始位置为2006（BEGIN前面），结束位置为2298（COMMIT后面）。

执行插入语句来重放数据：
```bash
mysqlbinlog --start-position=2006 --stop-position=2298 -d testdb /mysql/data/binlog/mysql-bin.000509 | mysql -uroot -h127.0.0.1
ERROR 1782 (HY000) at line 26: @@SESSION.GTID_NEXT cannot be set to ANONYMOUS when @@GLOBAL.GTID_MODE = ON.
```
因为我们开启了GTID模式，这里提示需要设置GTID_NEXT。

查看SET GTID_NEXT语句对应的pos为1927：
```bash
root@(none)> show binary logs;
+------------------+-----------+-----------+
| Log_name         | File_size | Encrypted |
+------------------+-----------+-----------+
| mysql-bin.000509 |      2702 | No        |
| mysql-bin.000510 |       197 | No        |
+------------------+-----------+-----------+
2 rows in set (0.00 sec)

root@(none)> show binlog events in 'mysql-bin.000509' from 760;
+------------------+------+-------------+-----------+-------------+--------------------------------------------------------------------------+
| Log_name         | Pos  | Event_type  | Server_id | End_log_pos | Info                                                                     |
+------------------+------+-------------+-----------+-------------+--------------------------------------------------------------------------+
| mysql-bin.000509 |  760 | Gtid        |       100 |         839 | SET @@SESSION.GTID_NEXT= '3e863e10-eede-11ed-a3fd-005056a3eca7:12722159' |
...
| Log_name         | Pos | Event_type     | Server_id | End_log_pos | Info                                                                                                                                                                    |
+------------------+-----+----------------+-----------+-------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| mysql-bin.000510 |   4 | Format_desc    |       100 |         126 | Server ver: 8.0.30, Binlog ver: 4                                                                                                                                       |
| mysql-bin.000510 | 126 | Previous_gtids |       100 |         197 | 3e863e10-eede-11ed-a3fd-005056a3eca7:1-12722163                                                                                                                         |
| mysql-bin.000510 | 197 | Gtid           |       100 |         287 | SET @@SESSION.GTID_NEXT= '3e863e10-eede-11ed-a3fd-005056a3eca7:12722164'                                                                                                |
| mysql-bin.000510 | 287 | Query          |       100 |         364 | BEGIN                                                                                                                                                                   |
| mysql-bin.000510 | 364 | Rows_query     |       100 |         553 | # BINLOG '
YqiXZBNkAAAAOwAAAKUIAAAAAH0AAAAAAAUABnRlc3RkYgACdDEAAwgPAwKQAQIBAYACA/z/AFLV
EXc=
YqiXZB5kAAAANgAAANsIAAAAAH0AAAAAAAEAAgAD/wAEAAAAAAAAAAQAUkRSMlgAAACfaI5Y
' |
| mysql-bin.000510 | 553 | Table_map      |       100 |         612 | table_id: 125 (testdb.t1)                                                                                                                                               |
| mysql-bin.000510 | 612 | Write_rows     |       100 |         666 | table_id: 125 flags: STMT_END_F                                                                                                                                         |
| mysql-bin.000510 | 666 | Xid            |       100 |         697 | COMMIT /* xid=1727 */                                                                                                                                                   |
+------------------+-----+----------------+-----------+-------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
8 rows in set (0.00 sec)
```
数据恢复成功，并且自动分配了GTID 12722164。

## 替换binlog中的删除事件
更为常见的情形时需要找到删除动作对应的DELETE语句，并将其统一替换为INSERT语句来进行恢复。需要数据库开启了`binlog_rows_query_log_events`参数。
